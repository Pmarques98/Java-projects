...
public class LeitorDeCsv implements LeitorBibliotecaDAO{	
	...
	public List<Livro> getAllLivros(String nomeArquivoLivros) {
		...
		VerificaExecao check = new VerificaExecao(null,livros,null,null);
		check.verificaRepeticao();
		reader.close();
		csvReader.close();
		}catch(Exception e) {
			e.getMessage();
	...
	public List<Exemplar> getAllExemplares(String nomeArquivoExemplares) {
		...
		VerificaExecao check = new VerificaExecao(null,livros,exemplares,null);
		check.verificaExemplar();
		reader.close();
		csvReader.close();
		}catch(Exception e) {
			e.getMessage();
	...
	public List<Emprestimo> getAllEmprestimos(String nomeArquivoEmprestimos) {
		...
		VerificaExecao check = new VerificaExecao(usuarios,null,exemplares,emprestimos);
		check.verificaEmprestimo();
		check.verificaUsuario();
		reader.close();
		csvReader.close();
		}catch(Exception e) {
			e.getMessage();
